...
pub struct UserPasswordUpdateDto {
    ...
    #[validate(
        custom(function = "validate_password_length"),
...
fn validate_password_length(password: &str) -> Result<(), ValidationError> {
    if password.len() < 1 {
        let mut error = ValidationError::new("password_required");
        error.message = Some("Password is required.".into());
        return Err(error);
    }

    if password.len() < 6 {
        let mut error = ValidationError::new("password_length");
        error.message = Some("Password must be at least 6 characters".into());
        return Err(error);
    }
